...
from dealer_purchase.admin import CarAdmin
from dealer_purchase.models import Car
...
class AdminTests(TestCase):
    ...
    def test_driver_license_number_listed(self) -> None:
        url = reverse("admin:dealer_service_driver_changelist")
        res = self.client.get(url)

        self.assertContains(res, self.driver.license_number)

    def test_driver_detail_license_number_listed(self) -> None:
        url = reverse("admin:dealer_service_driver_change", args=[self.driver.id])
        res = self.client.get(url)

        self.assertContains(res, self.driver.license_number)

    def test_driver_create_additional_info_listed(self) -> None:
        url = reverse("admin:dealer_service_driver_add")
        res = self.client.get(url)
